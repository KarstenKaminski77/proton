...
$oPdf->AddFont('dejavuserif',  'BI',   'DejaVuSerif-BoldItalic.ttf', true);
	...
	$oPdf->Cell(190,10,'SALES OFFER','','','R');
	$oPdf->Ln(10);
...
$oPdf->Ln(20);
	...
	$oPdf->MultiCell(190,7, 'Dear ' .$row_supplier['BuyerName'] .'
...
'. $_SESSION['content'],'','L');
	...
	$oTable->initialize(array(140,25,25),$aCustomConfiguration);
	...
	$aRow[1]['BORDER_TYPE'] = 'LRTB';
	$aRow[1]['TEXT'] = $row_products['PackSize']; 
	$aRow[1]['TEXT_ALIGN'] = "L";
	$aRow[1]['BORDER_COLOR'] = array(102,102,102);

	$aRow[2]['BORDER_TYPE'] = 'LRTB';
	$aRow[2]['TEXT'] = $row_products['Grade']; 
	$aRow[2]['BORDER_COLOR'] = array(102,102,102);
	$aRow[2]['TEXT_ALIGN'] = 'L';

	$oTable->addRow($aRow);
	
	$_SESSION['datasheet'] = $row_products['DataSheet'];
	$_SESSION['cao'] = $row_products['CAO'];
	...
	if(isset($_GET['Preview'])){
		...
		$pdf = 'Proton Chem Offer #'.$sourceid.' - '.$supplierid.'.pdf';
		$item = $_GET['Item'];
		
		mysqli_query($con, "UPDATE tbl_offers SET PDF = '$pdf' WHERE RFQId = '$item'")or die(mysqli_error($con));
		//$oPdf->Output();
		$oPdf->Output('pdf/'.$pdf);
				
	}
	
header('Location:pdf/mail-offer.php?Id='. $sourceid .'&Supplier='.$supplierid .'&Item='. $_GET['Item']);
