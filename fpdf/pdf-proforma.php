...
WHERE
	tbl_proforma_items.ProformaNo = '$proforma_no'";
...
if($_GET['Type'] == 2){
	...
	SELECT
		tbl_proforma.Date,
		tbl_proforma.PaymentTerms,
		tbl_proforma.Currency,
		tbl_proforma.DueDate,
		tbl_proforma.PO,
		tbl_proforma.Currency,
	...
	FROM
		tbl_proforma
	INNER JOIN tbl_qs_items ON tbl_proforma.QuoteNo = tbl_qs_items.QuoteNo
	INNER JOIN tbl_companies ON tbl_qs_items.SupplierId = tbl_companies.Id
	WHERE
		tbl_proforma.Id = '$proforma_no'";
...
$query_transport = mysqli_query($con, "SELECT * FROM tbl_transport WHERE ProformaNo = '$proforma_no'")or die(mysqli_error($con));
$row_transport = mysqli_fetch_array($query_transport);

$transport = $row_transport['Price'];

proforma_pdf_subtotal($con, $proforma_no);
proforma_pdf_vat($con, $proforma_no, $transport);
proforma_pdf_total($con, $proforma_no, $transport);
...
$aRow[1]['BORDER_TYPE'] = 'LRTB';
$aRow[1]['TEXT'] = $row_invoice_to['Date']; 
$aRow[1]['BACKGROUND_COLOR'] = array(255,255,255);
$aRow[1]['TEXT_COLOR'] = array(0,0,0);
$aRow[1]['TEXT_TYPE'] = '';
$aRow[1]['BORDER_COLOR'] = array(102,102,102);
...
$aRow[3]['BORDER_TYPE'] = 'LRTB';
$aRow[3]['TEXT'] = $row_invoice_to['DueDate']; 
$aRow[3]['BACKGROUND_COLOR'] = array(255,255,255);
$aRow[3]['TEXT_COLOR'] = array(0,0,0);
$aRow[3]['TEXT_TYPE'] = '';
$aRow[3]['BORDER_COLOR'] = array(102,102,102);
...
if(isset($_GET['Preview'])){
	...
	$pdf = 'Proton Chem Proforma Invoice #'.$proforma_no.'.pdf';
			
	mysqli_query($con, "UPDATE tbl_proforma SET PDF = '$pdf' WHERE Id = '$proforma_no'")or die(mysqli_error($con));
	...
	header('Location:pdf/mail-proforma.php?Id='. $proforma_no .'&Company='. $row_invoice_to['CompanyId'].$param);
