...
GROUP BY
	proton_db.tbl_product_company_relation.CompanyId
ORDER BY
	proton_db.tbl_companies.CompanyName ASC
";
...
ORDER BY tbl_products.`Name` ASC")or die(mysqli_error($con));
    ...
    <form id="form1" name="form1" method="post" action="rfq.php">
      <div id="breadcrumbs">
          ...
          <tr>
            <td colspan="2" class="tab"><table border="0" cellpadding="0" cellspacing="0">
                <tr>
                  ...
                  <td><img src="../images/icons/bread-crumb.png" width="45" height="35" /></td>
                  <td><a href="../Admin/Industries/index.php" class="breadcumbs">Source New Product</a></td>
                  <td><img src="../images/icons/bread-crumb.png" width="45" height="35" /></td>
                </tr>
            </table></td>
          </tr>
        ...
        <tr>
		  ...
		  ?>
          <select name="buyer" class="tarea-100" id="buyer" <?php echo $class; ?>>
          <option value="" <?php echo $class; ?>>Select A Buyer...</option>
          <?php
		  $i = 1;

		  while($row_buyer = mysqli_fetch_array($query_buyer)){

		  $i++;

		  ?>
          <option value="<?php echo $row_buyer['CompanyId']; ?>" <?php dd_list('#e6e6e6','#f3f2f2',$font,$i); ?>><?php echo $row_buyer['CompanyName'] .' - '. $row_buyer['BuyerName']; ?></option>
          <?php } ?>
          </select>
          </td>
        </tr>
        <tr>
		  ...
		  if($i == $numrows_products){
            ...
            <!-- Products -->
            <div id="industry-list-source">
            <label for="check_<?php echo $c; ?>">
              <table border="0" cellpadding="2" cellspacing="3" width="100%">
                <tr>
                  <td title="<?php echo $row_products['Name']; ?>"><?php echo word_limit($row_products['Name'], 20); ?>
                  <input name="id[]" type="hidden" id="id[]" value="<?php echo $row_products['ProductId']; ?>" /></td>
                  <td width="40">
                  <input name="qty[]" type="text" class="tarea-qty-source" id="qty_<?php echo $c; ?>" onfocus="if(this.value=='0'){this.value=''}" onblur="if(this.value=='kg'){this.value='0'}" value="0" style=" visibility:hidden" /></td>
                  <td width="15">
                  <input type="checkbox" name="product[]" id="check_<?php echo $c; ?>" value="<?php echo $row_products['ProductId']; ?>" <?php if($rows == 1){ echo 'checked="checked"'; } ?> title="Select Product" onchange="ShowHide('check_<?php echo $c; ?>','qty_<?php echo $c; ?>')" /></td>
                </tr>
              </table>
            </label>
            </div>
